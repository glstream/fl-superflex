...
, ktc.ktc_player_id
, pl.player_position
, ktc.sf_positional_rank
, ktc.sf_rank
, coalesce(ktc.sf_value, 1) as player_value
, pl.age
, RANK() OVER (PARTITION BY lp.user_id, pl.player_position ORDER BY coalesce(ktc.sf_value, 0) desc) as player_order
, qb_cnt
, rb_cnt
, wr_cnt
, te_cnt
...
, abs(coalesce(cast(round(cast(CASE 
	WHEN tp.player_position = 'QB' THEN ((ktc.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'RB' THEN ((ktc.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'WR' THEN ((ktc.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'TE' THEN ((ktc.sf_value/ptc.pos_max::float)*100) 
...
,coalesce(ktc.sf_value, 0) as player_value
,coalesce(ktc.sf_positional_rank,0) as positional_rank
,coalesce(ktc.sf_rank,0) as rank
from (select 
		user_id
		,ap.player_id
		,ap.ktc_player_id
...
full outer join (select position, max(ktc.sf_value) as pos_max from dynastr.ktc_player_ranks ktc group by position) ptc on ktc.position = ptc.position
