WITH base_players as (SELECT
                    ...
                    , fc.sf_position_rank
                    , fc.sf_overall_rank
                    , coalesce(fc.sf_value, -1) as player_value
                    , pl.age
                    , RANK() OVER (PARTITION BY lp.user_id, pl.player_position ORDER BY coalesce(fc.sf_value, -1) desc) as player_order
                    , qb_cnt
                    , rb_cnt
                    , wr_cnt
                    , te_cnt
                    ...
                    , abs(coalesce(cast(round(cast(CASE 
                        WHEN tp.player_position = 'QB' THEN ((fc.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'RB' THEN ((fc.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'WR' THEN ((fc.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'TE' THEN ((fc.sf_value/ptc.pos_max::float)*100) 
                    ...
                    ,coalesce(fc.sf_value, -1) as player_value
                    ,coalesce(cast(fc.sf_position_rank as integer),0) as positional_rank
                    ,coalesce(cast(fc.sf_overall_rank as integer),0) as rank
                    from (select 
                            user_id
                            ,ap.player_id
                            ,ap.fc_player_id
                    ...
                    full outer join (select player_position , max(fc.sf_value) as pos_max from dynastr.fc_player_ranks fc group by player_position) ptc on fc.player_position = ptc.player_position
