WITH base_players as (SELECT
                    ...
                    , abs(coalesce(cast(round(cast(CASE 
                        WHEN tp.player_position = 'QB' THEN ((dp.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'RB' THEN ((dp.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'WR' THEN ((dp.sf_value/ptc.pos_max::float)*100) 
                        WHEN tp.player_position = 'TE' THEN ((dp.sf_value/ptc.pos_max::float)*100) 
                    ...
                    full outer join (select player_position, max(dp.sf_value) as pos_max from dynastr.dp_player_ranks dp group by player_position) ptc on dp.player_position = ptc.player_position
