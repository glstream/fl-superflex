...
, sf.ktc_player_id
, pl.player_position
, sf.sf_positional_rank
, sf.sf_rank
, coalesce(sf.sf_value, 1) as player_value
, pl.age
, RANK() OVER (PARTITION BY lp.user_id, pl.player_position ORDER BY coalesce(sf.sf_value, 0) desc) as player_order
, qb_cnt
, rb_cnt
, wr_cnt
, te_cnt
...
LEFT JOIN dynastr.sf_player_ranks sf on pl.full_name  = sf.player_full_name
...
and pl.player_position IN ('QB', 'RB', 'WR', 'TE' ))
                    ...
                    , base_picks as (SELECT t1.user_id
                                ...
                                LEFT JOIN dynastr.sf_player_ranks sf on t1.player_name = sf.player_full_name
                                    )						   
                    , starters as (SELECT  
                    qb.user_id
                    , qb.player_id
...
,p.age
, abs(cast(round(cast(CASE 
	WHEN tp.player_position = 'QB' THEN (((40-p.age)/19::float)*100) 
	WHEN tp.player_position = 'RB' THEN (((31-p.age)/10::float)*100)
	WHEN tp.player_position = 'WR' THEN (((34-p.age)/13::float)*100) 
	WHEN tp.player_position = 'TE' THEN (((34-p.age)/13::float)*100) 
	end as integer), -1)/10 as integer) -10)  as age_tile
, abs(coalesce(cast(round(cast(CASE 
	WHEN tp.player_position = 'QB' THEN ((sf.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'RB' THEN ((sf.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'WR' THEN ((sf.sf_value/ptc.pos_max::float)*100) 
	WHEN tp.player_position = 'TE' THEN ((sf.sf_value/ptc.pos_max::float)*100) 
end as integer), -1)/10 as integer),0) -10) as val_tile
,p.team
,tp.player_id as sleeper_id
,tp.player_position
,tp.fantasy_position
,tp.fantasy_designation
,coalesce(sf.sf_value, 0) as player_value
,coalesce(sf.sf_positional_rank,0) as positional_rank
,coalesce(sf.sf_rank,0) as rank
from (select 
		user_id
		,ap.player_id
		,ap.ktc_player_id
...
full outer join (select _position as position, max(sf.sf_value) as pos_max from dynastr.sf_player_ranks sf group by _position) ptc on sf._position = ptc.position
